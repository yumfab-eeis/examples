...
class NoiseResGenetator(nn.Module):
    def __init__(self, block, nblocks, nchannels, nfilters, nclasses, level):
        ...
        self.in_planes = nfilters
        self.pre_layers = nn.Sequential(
            nn.ConvTranspose2d(  self.in_planes, 4*nfilters, 4, 1, 0, bias=False),
            nn.BatchNorm2d(4*nfilters),
            nn.ReLU(True),
        )
        self.layer1 = self._make_layer(block, 4*nfilters, 4*nfilters, nblocks[0], level=level)
        self.layer2 = self._make_layer(block, 4*nfilters, 8*nfilters, nblocks[1], level=level)
        self.layer3 = self._make_layer(block, 8*nfilters, 8*nfilters, nblocks[2], level=level)
        self.layer4 = self._make_layer(block, 8*nfilters, 4*nfilters, nblocks[3], level=level)
        self.layer5 = self._make_layer(block, 4*nfilters, nchannels, nblocks[4], level=level, isLastBN=False)
...
def noiseresgenerator101(nchannels, nfilters, nclasses, level=0.7):
    return NoiseResGenetator(NoiseNoPoolBlock, [4,2,2,2,8], nchannels=nchannels, nfilters=nfilters, nclasses=nclasses, level=level)
