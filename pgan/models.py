...
class NoiseGenetator(nn.Module):
    def __init__(self, block, nblocks, nchannels, nfilters, nclasses, level):
        ...
        self.in_planes = nfilters
        self.pre_layers = nn.Sequential(
            nn.ConvTranspose2d(  self.in_planes, 32*nfilters, 4, 1, 0, bias=False),
            nn.BatchNorm2d(32*nfilters),
            nn.ReLU(True),
        )
        self.layer1 = self._make_layer(block, 32*nfilters, 16*nfilters, nblocks[0], level=level)
        ...
        self.layer5 = self._make_layer(block, 4*nfilters, nchannels, nblocks[4], level=level)
