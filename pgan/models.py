...
class NoiseGenetator(nn.Module):
    def __init__(self, block, nblocks, nchannels, nfilters, nclasses, level):
        ...
        self.layer1 = self._make_layer(block, 16*nfilters, 64*nfilters, nblocks[0], level=level)
        self.layer2 = self._make_layer(block, 64*nfilters, 32*nfilters, nblocks[1], level=level)
        self.layer3 = self._make_layer(block, 32*nfilters, 16*nfilters, nblocks[2], level=level)
        self.layer4 = self._make_layer(block, 16*nfilters, 8*nfilters, nblocks[3], level=level)
        self.layer5 = self._make_layer(block, 8*nfilters, nchannels, nblocks[4], level=level)
