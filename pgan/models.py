...
class NoiseGenetator(nn.Module):
    def __init__(self, block, nblocks, nchannels, nfilters, nclasses):
        ...
        self.layer3 = self._make_layer(block, 8*nfilters, 4*nfilters, nblocks[2], level=0.4)
        self.layer4 = self._make_layer(block, 4*nfilters, 1*nfilters, nblocks[3], level=0.6)
        self.layer5 = self._make_layer(block, 1*nfilters, nchannels, nblocks[4], level=1.0)
...
def noisegenerator101(nchannels, nfilters, nclasses):
    return NoiseGenetator(NoiseBasicBlock, [1,1,1,1,1], nchannels=nchannels, nfilters=nfilters, nclasses=nclasses)
